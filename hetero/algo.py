'''
algo.py contains the implementation of the algorithm.

The functions are named according to their intended purpose.
'''
...
def beta_from_linreg(data, Z, U, discount):
    '''
    Solve the linear regression problem to get beta.
    '''
    v = Z - discount * U
...
class MCPImpl(object):
    '''
    MCPImpl is the implementation of MCP algorithm. 
    Details in Appendix A Computation of the paper. 
    '''
...
class BetaOptimizer(object):
    '''
    BetaOptimizer is the implementation of Appendix A Computation of the paper. 
    '''
    def __init__(
        self,
        data,
...
def group(betas, config):
    '''
    Clustering the betas into groups.
    '''
...
def align_binary_labels(labels, truth, threshold=1.0 / 2):
    '''
    Utility function to combine labels and other covariates of the data for later calculation.
    '''
    assert (
        len(labels.shape) == 1
